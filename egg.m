function egg()
    ...
    [xmin, xmax, ymin, xmax] = find_bounding_box(x0, y0, theta, egg_params)

    plot ([x_min, xmax, xmax, xmin, xmin])
end
...
function[xmin, xmax, ymin, ymax] = find_bounding_box(x0, y0, theta, egg_params)
    ...
    for s_guess = s_guess_list
            s_rootx = orion_secant(egg_wrapper_func2_x, s_guess, s_guess + 1e-4, dxtol, ytol, max_iter, dfdxmin);
            [V, G] = egg_func(s, s_rootx, y0, theta, egg_params);
            x_list(end + 1) = V(1);

            s_rooty = orion_secant(egg_wrapper_func2_y, s_guess, s_guess+1e-4, dxtol, ytol, max_iter, dfdxmin);
            [V, G] = egg_func(s, s_rooty, y0, theta, egg_params);
            x_list(end + 1) = V(2);
    end
    xmin = min(s_list);
    xmax = max(x_list);
